# Hello, my name is **Janine Tavares Cunha** — Web Developer in Training 👩‍💻

🎓 Technology and Systems Development Student  
💻 Focused on Full Stack Web Developer | Back-End Developer 

🌎 Available for remote work  
🚀 Seeking an opportunity as an Intern  

🌎 Versão em português → [Clique aqui](README.md)

---

# 🚀 About Me

My interest in technology started at **11 years old**, when I had my first contact with basic computer skills, using tools such as **Windows, Word, Excel and PowerPoint**.

Over the years I faced several challenges in the job market, until I discovered in technology a real opportunity for professional transformation.

This change began when I joined the **Recode Pro AI** program, which provided structured technical training and the opportunity to start a new career in technology.

During my software development training, I deepened my knowledge in:

- Web Development
- Programming Logic
- Data Structures
- Databases

I learned fundamental technologies such as **HTML, CSS and JavaScript**, used to build interfaces and web applications.

I completed my professional technology training through the **Recode Pro AI program**, with **more than 540 hours of training**, developing practical projects using modern web development technologies.

During this period I developed applications involving:

- Front-end
- Back-end
- APIs
- Databases

Through the program I also expanded my knowledge in **backend technologies such as PHP and Java**, as well as **databases like MySQL**.

Currently, I continue studying and developing personal projects to strengthen my portfolio and grow as a software developer.

---

# 📚 Education

📚 Technology Training — **Recode Pro AI (540h)**  
📚 Basic Computer Course  
📚 High School Diploma

---

# 🛠 Technologies and Tools

## Languages

- HTML5
- CSS3
- JavaScript
- TypeScript
- Java
- PHP
- Node.js

## Frameworks and Libraries

- ReactJS
- Bootstrap

## Databases

- MySQL
- MongoDB

## Tools

- Git
- GitHub
- VS Code
- Eclipse
- Docker (basic)

---

# 📌 Featured Projects

### 🧾 Mini ERP
Management system developed to organize business operations.

### ✈️ Travel Agency Website
Web project focused on presenting travel destinations and tourism services.

### 📍 Local Business Recognition Platform
Project focused on promoting and valuing local businesses.

### 💰 Payroll System
React application developed to simulate payroll calculations.

### 🌿 Indigenous Platform – Health and Medicinal Plants
Web application created to promote and value traditional indigenous knowledge.

### ⚙️ Back-end API – Node.js
Back-end application developed for a technical test, focused on route creation and API structure.

### 📊 Web Dashboard – GDASH
Web dashboard developed as a technical challenge, focused on visual data organization.

### 💄 Makeup Store – Complete E-commerce
Web-based e-commerce application for selling cosmetic products, built using PHP, JavaScript, Bootstrap, and MySQL.

### 📸 Foto&Moda – Portfolio Platform
Full Stack system for managing fashion photography portfolios. The platform allows photographers to showcase their work, organize galleries, and publish fashion editorials.

### 🚢 Cruises & Tourism – Maritime Travel Agency Web System
Complete Full Stack web system built with a simplified MVC architecture, developed to simulate a travel agency specialized in international cruise tourism.

---

# 💡 Areas of Interest

- Web Development
- APIs and Back-end
- Full Stack Applications
- Digital solutions for businesses
- Technology applied to education and culture

---

# 🌎 Languages

English — Basic (learning)  
Spanish — Intermediate (learning)

---

# 🎯 Career Goal

I am looking for an opportunity as a **Full Stack Web Developer | Back-End Developer or Intern**, where I can apply my knowledge, grow technically and contribute to the development of well-structured software solutions.

---

# 📫 Contact

LinkedIn  
https://www.linkedin.com/in/janine-cunha-74b456307

Email  
janinecunha213@gmail.com
